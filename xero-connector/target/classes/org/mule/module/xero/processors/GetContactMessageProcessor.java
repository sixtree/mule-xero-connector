...
/**
 * GetContactMessageProcessor invokes the {@link org.mule.module.xero.XeroConnector#getContact(java.lang.String)} method in {@link XeroConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class GetContactMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private String _contactIdType;
    private static Logger logger = LoggerFactory.getLogger(GetContactMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getContact"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            String _transformedContactId = ((String) evaluateAndTransform(_muleMessage, GetContactMessageProcessor.class.getDeclaredField("_contactIdType").getGenericType(), null, contactId));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getContact"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.getContact(_transformedContactId);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("getContact"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
