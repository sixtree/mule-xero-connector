...
/**
 * GetAccountMessageProcessor invokes the {@link org.mule.module.xero.XeroConnector#getAccount(java.lang.String)} method in {@link XeroConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class GetAccountMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{

    private Object accountId;
    private String _accountIdType;
    private static Logger logger = LoggerFactory.getLogger(GetAccountMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getAccount"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            String _transformedAccountId = ((String) evaluateAndTransform(_muleMessage, GetAccountMessageProcessor.class.getDeclaredField("_accountIdType").getGenericType(), null, accountId));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getAccount"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.getAccount(_transformedAccountId);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("getAccount"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
