...
/**
 * GetManualJournalMessageProcessor invokes the {@link org.mule.module.xero.XeroConnector#getManualJournal(java.lang.String)} method in {@link XeroConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class GetManualJournalMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{

    private Object manualJournalId;
    private String _manualJournalIdType;
    private static Logger logger = LoggerFactory.getLogger(GetManualJournalMessageProcessor.class);
    /**
     * Module object
     * 
     */
     ...
     * @param value Value to set
     */
    public void setManualJournalId(Object value) {
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getManualJournal"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            String _transformedManualJournalId = ((String) evaluateAndTransform(_muleMessage, GetManualJournalMessageProcessor.class.getDeclaredField("_manualJournalIdType").getGenericType(), null, manualJournalId));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getManualJournal"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.getManualJournal(_transformedManualJournalId);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("getManualJournal"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
