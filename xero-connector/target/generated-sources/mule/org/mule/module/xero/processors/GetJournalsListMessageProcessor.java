...
/**
 * GetJournalsListMessageProcessor invokes the {@link org.mule.module.xero.XeroConnector#getJournalsList(java.lang.String, java.lang.String)} method in {@link XeroConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class GetJournalsListMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(GetJournalsListMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getJournalsList"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            String _transformedWhereClause = ((String) evaluateAndTransform(_muleMessage, GetJournalsListMessageProcessor.class.getDeclaredField("_whereClauseType").getGenericType(), null, whereClause));
            String _transformedOrderBy = ((String) evaluateAndTransform(_muleMessage, GetJournalsListMessageProcessor.class.getDeclaredField("_orderByType").getGenericType(), null, orderBy));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getJournalsList"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.getJournalsList(_transformedWhereClause, _transformedOrderBy);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("getJournalsList"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
