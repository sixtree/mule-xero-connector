...
/**
 * GetEmployeeMessageProcessor invokes the {@link org.mule.module.xero.XeroConnector#getEmployee(java.lang.String)} method in {@link XeroConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class GetEmployeeMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{

    private Object employeeId;
    private String _employeeIdType;
    private static Logger logger = LoggerFactory.getLogger(GetEmployeeMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getEmployee"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            String _transformedEmployeeId = ((String) evaluateAndTransform(_muleMessage, GetEmployeeMessageProcessor.class.getDeclaredField("_employeeIdType").getGenericType(), null, employeeId));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getEmployee"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.getEmployee(_transformedEmployeeId);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("getEmployee"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
