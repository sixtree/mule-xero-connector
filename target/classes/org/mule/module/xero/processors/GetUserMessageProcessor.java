...
/**
 * GetUserMessageProcessor invokes the {@link org.mule.module.xero.XeroConnector#getUser(java.lang.String)} method in {@link XeroConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(GetUserMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getUser"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            String _transformedUserId = ((String) evaluateAndTransform(_muleMessage, GetUserMessageProcessor.class.getDeclaredField("_userIdType").getGenericType(), null, userId));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getUser"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.getUser(_transformedUserId);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("getUser"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
