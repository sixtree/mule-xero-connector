...
/**
 * GetJournalMessageProcessor invokes the {@link org.mule.module.xero.XeroConnector#getJournal(java.lang.String)} method in {@link XeroConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class GetJournalMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private String _journalIdType;
    private static Logger logger = LoggerFactory.getLogger(GetJournalMessageProcessor.class);
    /**
     * Module object
     * 
     */
     ...
     * @param value Value to set
     */
    public void setJournalId(Object value) {
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getJournal"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            String _transformedJournalId = ((String) evaluateAndTransform(_muleMessage, GetJournalMessageProcessor.class.getDeclaredField("_journalIdType").getGenericType(), null, journalId));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getJournal"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.getJournal(_transformedJournalId);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("getJournal"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
